...
Ext.define('Ext.layout.container.Box', {
    ...
    beginLayout: function (ownerContext) {
        ...
        // Capture whether the owning Container can scroll.  The presence of a scroller
        // indicates that the container has the potential to scroll, either by the user,
        // or programmatically
        me.canScroll = !!owner.scrollable;

        // Capture whether the owning Container is user scrollable in the parallel direction
        me.scrollParallel = owner.scrollFlags[names.x];

        // Capture whether the owning Container is user scrollable in the perpendicular direction
        me.scrollPerpendicular = owner.scrollFlags[names.y];

        // If we can scroll parallel, capture the scroll position of the encapsulating element
        if (me.canScroll) {
    ...
    completeLayout: function(ownerContext) {
        ...
        // If we can scroll parallel, restore the saved scroll position
        if (me.canScroll) {
    ...
    publishInnerCtSize: function(ownerContext, reservedSpace) {
        var me = this,
            ...
            innerCtWidth = (ownerContext.parallelSizeModel.shrinkWrap || (plan.tooNarrow && me.canScroll)
