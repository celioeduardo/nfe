    ...
    doFire: function() {
        ...
        for (i = 0,ln = firingListeners.length; i < ln; i++) {
            ...
            if (options.delegate && me.info.targetType === 'element') {
                // findParent is required because the event may have fired on something inside the delegate, consider:
                // <div id="a">
                //     <div id="b" class="foo">
                //         <div id="c">Clicked</div>
                //     </div>
                // </div>
                // a.on('click', fn, null, {delegate: '.foo'});
                // 
                // The target that is passed to the event handler should be b, however the target on the event
                // object should remain as c.
                // substring used to strip the '#' + prefix from the element id.
                firingArguments[1] = Ext.fly(arg1).findParent(info.target, observable.getObservableId().substring(1));
            }
