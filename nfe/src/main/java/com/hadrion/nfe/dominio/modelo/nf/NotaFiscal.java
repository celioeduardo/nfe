...
public class NotaFiscal {
	...
	@AssociationOverrides({ @AssociationOverride(name = "observacoes", joinTable = @JoinTable(name = "FISCO_OBS")) })
	private Informacao informacaoFisco;

	@Embedded
	@AttributeOverride(name = "texto", column = @Column(name = "CONTRIB_INFORMACAO", length = 4000))
	@AssociationOverrides({ @AssociationOverride(name = "observacoes", joinTable = @JoinTable(name = "CONTRIB_OBS")) })
	...
	@JoinTable(name = "REFERENCIAS", joinColumns = @JoinColumn(name = "ID_NF"), inverseJoinColumns = @JoinColumn(name = "ID_REFERENCIA"))
	...
	@Embedded
	@AttributeOverrides({ @AttributeOverride(name = "numero", column = @Column(name = "CANC_NUMERO_PROTOCOLO")) })
	...
	public NotaFiscal(Ambiente ambiente, NotaFiscalId notaFiscalId,
			FilialId filialId, String naturezaOperacao,
			FormaPagamento formaPagamento, Modelo modelo, Serie serie,
			Long numero, Date emissao, Date dataHora, Integer codigoNumerico,
			FormatoDanfe formatoDanfe, TipoEmissao tipoEmissao,
			TipoOperacao tipoOperacao, LocalDestino localDestino,
			Municipio municipioFatoGerador, boolean consumidorFinal,
			Finalidade finalidade, Presenca presenca, Processo processo,
			List<Referencia> referencias, Emitente emitente,
			Destinatario destinatario, LocalRetirada localRetirada,
		...
		this.codigoNumerico = codigoNumerico == null ? randInt(1, 99999999)
				: codigoNumerico;
	...
	public boolean pendenteDeTransmissao() {
		return this.getSituacao() == Situacao.EMITIDA
				|| this.getSituacao() == Situacao.INDEFINIDA;
	}

	public void emitida() {
		assertSituacaoIgual("Situação inválida: " + this.getSituacao(),
				Situacao.INDEFINIDA);
	...
	public void autorizada(NumeroProtocolo numeroProtocolo, Mensagem mensagem,
		...
		DominioRegistro.eventoDominioPublicador().publicar(
				new NotaFiscalAutorizada(notaFiscalId(), chaveAcesso(),
						ambiente(), filialId()));
	...
	public Dinheiro totalDesconto() {
		Dinheiro result = Dinheiro.ZERO;
		for (Item item : getItens())
			result = result.soma(item.valorDescontoTotal());
		return result;
	}
	...
	public Dinheiro total() {
		return totalProdutos().subtrair(totalDesconto())
				.subtrair(valorIcmsDesonerado()).soma(totalIcmsSt())
				.soma(totalFrete()).soma(totalSeguro())
	...
	private ChaveAcesso gerarChaveAcesso() {
		return new ChaveAcesso(emitente().endereco().municipio().uf(),
				emissao(), emitente().cnpj(), serie(), numero(), tipoEmissao(),
				codigoNumerico());
	}
	...
	public void alterarTipoEmissaoParaContingencia(Contingencia contingencia) {
		setTipoEmissao(TipoEmissao.contingenciaPelaUf(ufEmitente()));
		this.contingencia = contingencia;
	}
	...
	private void setTipoEmissao(TipoEmissao novoTipoEmissao) {
		if (tipoEmissao == novoTipoEmissao)
			return;
	...
	private String consisteTrocaTipoEmissao(TipoEmissao novoTipoEmissao) {
		...
		if (danfeImpresso
				&& (tipoEmissao == TipoEmissao.FS_DA || tipoEmissao == TipoEmissao.FS_IA))
	...
	public void definirDanfeComoImpresso() {
		boolean fs = this.tipoEmissao() == TipoEmissao.FS_DA
				|| this.tipoEmissao() == TipoEmissao.FS_IA;
		if (!fs && !this.estaAutorizada())
			throw new RuntimeException(
					"Danfe somente pode ser impresso para nota autorizada.");
		danfeImpresso = true;
	}
	...
	private void mesclarInformacaoContribuinte(Informacao outra) {
		if (this.informacaoContribuinte == null)
			...
			this.informacaoContribuinte = this.informacaoContribuinte
					.mesclar(outra);
	}
	...
	public void alterarModoOperacao(ModoOperacao modoOperacao,
		...
		case FS_DA:
			...
			break;
		case SVC:
			TipoEmissao tipoEmissao = tipoEmissaoPeloModoOperacao(modoOperacao,
					ufEmitente());
	...
	public static TipoEmissao tipoEmissaoPeloModoOperacao(
			ModoOperacao modoOperacao, Uf ufEmitente) {
		switch (modoOperacao) {
		case NORMAL:
	...
	public void registrarCartaCorrecao(int sequencia, String correcao,
			Date dataRegistro, String xmlEnvio, String xmlRetorno) {
		...
		getCartasCorrecao().add(
				new CartaCorrecao(sequencia, correcao, dataRegistro, xmlEnvio,
						xmlRetorno));
	}

	public void registrarCartaCorrecao(String correcao, Date dataRegistro) {
		registrarCartaCorrecao(ultimaSequenciaCartaCorrecao() + 1, correcao,
				dataRegistro, null, null);
	}
