...
public class Notification extends AssertionConcern implements Serializable {
    ...
    public Notification(
            ...
            EventoDominio anEvent) {

        this();

        this.setEvent(anEvent);
        this.setNotificationId(aNotificationId);
        this.setOccurredOn(anEvent.ocorridoEm());
        this.setTypeName(anEvent.getClass().getName());
        this.setVersion(anEvent.versaoEvento());
    }

    @SuppressWarnings("unchecked")
    public <T extends EventoDominio> T event() {
        return (T) this.event;
    }
    ...
    protected void setEvent(EventoDominio anEvent) {
